...
func checkForRelease(bugs: Int, music: Bool, levels: Int) {
    ...
    if bugs >= 10 {
        print("MORE THAN 10 BUGS PRESENT")
     }
    if music == false{
        print("MUSIC REQUIRED TO CLEAR")
        }
    if levels <= 5{
        print("MORE THAN 5 LEVELS ")
         }
        }
...
func checkTrainingStatus(name: String, bike: Bool, run: Bool) {
    ...
    if bike == true && run == true{
        print("PERSON IS FIT AND GOOD TO GO!")
    }
    else{
        if bike != true {
            print("PERSON NEEDS TO WORK ON BIKING SKILLS")
        }
        if run != true {
            print("PERSON NEEDS TO WORK ON RUNNING SKILLS")
        }
    }
}
